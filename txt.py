...
FOOD_DATABASE = {
    ...
    # 간식/패스트푸드
    "라면 (1봉지, 120g)": {"calories": 500, "탄수화물": 75.0, "단백질": 10.0, "지방": 18.0, "na트륨": 1900, "칼슘": 10, "철분": 0.5},
...
with col_food_input:
    ...
    if user_food_input:
        ...
        if matches:
            ...
            # 실제 음식 이름만 추출
            recommended_food = selected_match_display.split(' (정확도')[0]
        else:
            st.warning("🤔 앗, 비슷한 음식을 찾기 어렵네요. 음식 이름을 정확하게 입력하거나 다른 음식을 시도해보세요.")
...
for meal in st.session_state.daily_meals:
    ...
    if food_name in FOOD_DATABASE:
        ...
        for nutrient, value in food_info.items():
            if nutrient in total_nutrients: # DAILY_RDA에 정의된 영양소만 합산
                total_nutrients[nutrient] += (quantity_g / base_g) * value
            elif nutrient == "calories": # 칼로리는 항상 합산
...
if not st.session_state.daily_meals:
    ...
    for i, nutrient in enumerate(main_nutrients_display):
        with main_cols[i]:
            ...
            if rda_val is not None:
                ...
                st.markdown(f"<p style='font-size:0.8em; color:gray;'>{nutrient} (권장량 정보 없음)</p>", unsafe_allow_html=True)
    ...
    for i, nutrient in enumerate(micro_nutrients_display):
        ...
        if rda_val is not None:
            with cols_micros[i % num_cols_micros]:
                unit = "mg"
                if nutrient in ["비타민A", "비타민D", "비타민K", "비タミンB12", "엽산"]:
                    unit = "mcg"
                elif nutrient == "오메가3":
                    unit = "g"
    ...
    if total_nutrients["탄수화물"] < DAILY_RDA["탄수화물"] * 0.7:
        tips.append("🧠 **뇌 활동과 에너지를 위한 탄수화물**이 조금 부족할 수 있어요. 쌀밥, 현미밥, 통밀빵, 고구마 등 복합 탄수화물을 충분히 먹어서 공부에 필요한 에너지를 채워주세요!")
    ...
    if total_nutrients.get("철분", 0) < DAILY_RDA["철분"] * 0.7:
